...
class Screenshot : Application() {
    ...
    override fun start(primaryStage: Stage) {
        ...
        slider.setBlockIncrement(1000.0);
        ...
        val panel : HBox = HBox(takeScreenshot, chooseFile, cropImage, save, cancel, paint, ch)
        val root : VBox = VBox(panel, slider)
        ...
        takeScreenshot.onAction = EventHandler {
                ...
                if (ch.isSelected) { primaryStage.hide() }
                ...
                if (ch.isSelected) { primaryStage.show() }
        }
        ...
        save.onAction = EventHandler {
            ...
            if (file != null) {
                ...
                if (isToCrop) { g2.clearRect(0.0, 0.0, canvas.width, canvas.height) }

                val pr : PixelReader = image.pixelReader
                wi = WritableImage(pr, 0, 0, width, height)
                if (isToPaint) { canvas.snapshot(SnapshotParameters(), wi) }
